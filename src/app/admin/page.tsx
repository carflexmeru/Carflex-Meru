...
export default function AdminAnalytics() {
  ...
  const METRICS = [
    { label: "Bazaar Inventory", value: data?.metrics.inventory, icon: "inventory_2", color: "text-black" },
  ...
  return (
    <div className="space-y-12 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {METRICS.map((m) => (
          <div key={m.label} className="bg-white border-4 border-black p-10 shadow-[10px_10px_0px_#0A0A0A] hover:shadow-[15px_15px_0px_#E60000] transition-all group">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1">{m.label}</p>
            <p className="text-5xl font-black tracking-tighter group-hover:text-primary transition-colors">{m.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="bg-white border-4 border-black p-12">
           <h3 className="text-xl font-black uppercase tracking-tighter mb-8 flex items-center gap-3 text-black">
             <span className="w-2 h-8 bg-black"></span>
             Infrastructure Health
          </h3>
          <div className="grid grid-cols-2 gap-8">
             <div className="space-y-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Database Latency</p>
                <p className="text-2xl font-black">12ms</p>
             </div>
             <div className="space-y-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Active Handshakes</p>
                <p className="text-2xl font-black text-green-500 font-mono">STABLE</p>
             </div>
             <div className="space-y-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Audit Redundancy</p>
                <p className="text-2xl font-black">ACTIVE</p>
             </div>
             <div className="space-y-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Node Sync</p>
