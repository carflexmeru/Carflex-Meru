...
export default function MeruRegistrationsPage() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const res = await fetch("/api/events/registrations?eventName=MERU_2026");
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-16 space-y-12">
      <div className="flex flex-col gap-3">
        <p className="text-[10px] font-black uppercase tracking-[0.5em] text-primary">Event Registration Registry</p>
        ...
        <p className="max-w-2xl text-zinc-500 font-bold uppercase tracking-widest text-[10px] leading-relaxed">
          Browse every vehicle registered under this event and download the ticket or QR code from one place.
        </p>
      </div>

      <div className="nm-card p-6 md:p-8 flex flex-wrap gap-4 items-center justify-between">
        <div>
          <p className="text-[9px] font-black uppercase tracking-widest text-zinc-500">Event</p>
          <p className="text-2xl font-black uppercase tracking-tighter text-foreground">MERU_2026</p>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div className="nm-inset p-4 text-center">
            <p className="text-[9px] font-black uppercase tracking-widest text-zinc-500 mb-1">Registered</p>
            <p className="text-3xl font-black text-foreground">{tickets.length}</p>
          </div>
          <div className="nm-inset p-4 text-center">
            ...
            <div key={ticket.id} className="nm-card p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="nm-card bg-zinc-900 text-white py-3 font-black uppercase tracking-widest text-[9px] border-none"
                >
                  Download QR
                </button>
                <Link
                  ...
                  className="nm-card bg-primary text-white py-3 font-black uppercase tracking-widest text-[9px] text-center border-none"
                >
                  Download Ticket
                </Link>
              </div>
